...
<head>
  ...
  <style>
    @media print {
      body { margin: 0; font-size: 12pt; background: white; }
    ...
    .action-btn {
      ...
      padding: 8px 16px; border-radius: 6px; font-weight: 500; transition: all 0.2s; cursor: pointer;
    }
    .action-btn:hover { transform: translateY(-1px); }
    .status-badge {
      padding: 4px 8px; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; text-transform: capitalize;
    }
    @media (max-width: 640px) {
      .action-buttons { display: flex; flex-direction: row; gap: 0.5rem; justify-content: flex-end; }
      .action-btn { width: 36px; height: 36px; border-radius: 50%; }
  ...
  <header class="bg-white shadow-sm no-print">
    <div class="max-w-7xl mx-auto px-4 py-3 sm:px-6 flex justify-between items-center">
      <div class="flex items-center space-x-3">
        <a href="{{ url('/') }}" class="text-gray-600 hover:text-gray-900" title="Kembali">
          <i class="fas fa-arrow-left text-lg"></i>
        </a>
        <h1 class="text-lg font-bold text-gray-800">Rekap Kehadiran Harian Guru</h1>
      </div>
    </div>
  </header>
